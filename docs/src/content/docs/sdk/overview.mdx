...
PHP, C# (0.16+), Swift (0.18+), JVM languages (0.20+) and Elixir (0.22+).
They all resolve
...
- **Elixir (0.22+)** uses a [Rustler](https://github.com/rusterlium/rustler)
  NIF that embeds the same resolver and exchanges the same JSON
  request/response shape as the C ABI.
...
[JVM languages (0.20+)](/sdk/jvm) and [Elixir (0.22+)](/sdk/elixir).
...
- **Elixir (0.22+)** downloads a precompiled Rustler NIF for the host platform
  through RustlerPrecompiled; glibc/musl Linux, Intel/Arm macOS, and x64
  Windows NIFs are included.
...
| Haskell (source) | ✓ | — | — | — | ✓ (0.17+) | — |
| JVM (0.20+) | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| Elixir (0.22+) | ✓ (glibc and musl) | ✓ (glibc and musl) | ✓ | ✓ | ✓ | — |
